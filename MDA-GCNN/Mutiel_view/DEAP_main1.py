...
def load_and_combine_deap_data(data_dir="G:/Experiment/Experiment3/Data/DEAP"):
    """
    加载并合并32个受试者的DEAP数据集数据和标签。

    :param data_dir: DEAP数据集存储目录的路径。
    :return: 合并后的特征数组和标签数组。
    """
    all_features = []
    all_labels = []
    ...
    for subject_id in range(1, 33):
        ...
        all_labels.append(arousal_labels)

    # 将数据和标签从列表转换为numpy数组并堆叠
...
def main():
    ...
    #     print("Label shape:", labels.shape)
        ...
        features, labels = load_and_combine_deap_data()


        # features = features[:25000, :, :]
        # labels = labels[:25000]
        print('combine feature',features.shape)
        print('combine labels',labels.shape)
