...
def optimize_adjacency(features, alpha=0.3, learning_rate=0.0001, iterations=100):
...
#     # 首先检查features是否为张量并且位于CUDA上
#     # if isinstance(features, torch.Tensor) and features.is_cuda:
#     #     features = features.cpu().numpy()  # 移至CPU并转换为NumPy数组
#     # elif isinstance(features, torch.Tensor):
#     #     features = features.numpy()  # 直接转换为NumPy数组
#     # 如果features已经是NumPy数组，则无需转换
#
#     N, D = features.shape
