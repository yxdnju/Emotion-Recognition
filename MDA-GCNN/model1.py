...
#
#     def process_and_convolve(self, batch_features_np):
#         # Assume process_batch is a function that converts batch_features_np to a Batch object
#         batch_data = process_batch(batch_features_np)
#         x = batch_data.x
#         edge_index = batch_data.edge_index
#         batch = batch_data.batch
#
#         x = F.relu(self.conv1(x, edge_index))
#         x = F.dropout(x, training=True)  # or self.training for dynamic behavior
...
# 定义图卷积网络模型
# EEGGraphConvNet model that uses GraphOperations
class EEGGraphConvNet(nn.Module):
    ...
    def forward(self, batch_features_np):
        ...
        # print('模型开始')
        # 打印每个张量的形状
        # for i, tensor in enumerate(batch_features_np):
        #     print(f'第 {i + 1} 个张量的形状：{tensor.size()}')
        # 直接调用GraphOperations的forward方法获取batch_data
        # if isinstance(batch_features_np, torch.Tensor):
        #     print(f'Tensor shape: {batch_features_np.shape}')
        # else:
        #     print(f'Data type: {type(batch_features_np)}')
        # print('输入模型前----')
